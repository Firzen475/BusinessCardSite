...
docker_init_database_dir() {
	...
	echo 'initdb --username="$POSTGRES_USER" --pwfile=<(echo "$POSTGRES_PASSWORD") '"$POSTGRES_INITDB_ARGS"' "$@"'
...
pg_setup_hba_conf() {
	cp "/etc/postgresql/${PG_MAJOR}/main/pg_hba.conf" /var/lib/postgresql/
	cp "/etc/postgresql/${PG_MAJOR}/main/pg_ident.conf" /var/lib/postgresql/
...
#	} >> "$PGDATA/pg_hba.conf"
	...
	sed -i -r 's:^local.*all.*postgres.*:local   all             postgres                                trust:' /var/lib/postgresql/pg_hba.conf
}
...
#_init_cron(){
#	echo "_init_cron"
#	sudo /var/lib/postgresql/start_cron.sh
#	echo "${SHEDULE} chmod +x /root/run.sh && su postgres /root/run.sh" >> /var/lib/postgresql/mycron
#	crontab -u postgres /var/lib/postgresql/mycron
#	crontab -l
#}
...
_main() {
	...
	if [ "$1" = 'postgres' ] && ! _pg_want_help "$@"; then
		...
		sed -i -r "s:PGDATA:$PGDATA:" /var/lib/postgresql/postgresql.conf
		pg_setup_hba_conf "$@"
		_clear_db
#		if [ "$DATABASE_ALREADY_EXISTS" = 'true' ]; then
#			ls -all /var/lib/postgresql/data/base/
#		fi
